...
export default function ScoreBoard({ difficulty, discColor }: ScoreBoardProps) {
  const [playerDiscImagePath, computerDiscImagePath] =
    discColor === "Black" ? [black_disc_imagePath, white_disc_imagePath] : [white_disc_imagePath, black_disc_imagePath];
  const [playerDiscImage, computerDiscImage] = discColor === "White" ? ["black", ""] : ["", "black"];
  return (
    <div className="scoreBoard">
        ...
        <div className="computerCard computerNameCard">Computer:{difficulty}</div>
      </div>
  );
}
